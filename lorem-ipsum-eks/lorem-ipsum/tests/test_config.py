...
class TestConfig:
    def test_config_valid(self, config_valid):
        _result = app.get_config()
        config = json.loads(_result.response[0].decode('utf-8'))
        print(config)
        assert config.get('aurora_host') == 'localhost'
        assert '200' == _result.status

    def test_config_no_sensitive_data(self, config_valid):
        _result = app.get_config()
        config = json.loads(_result.response[0].decode('utf-8'))
        assert '200' == _result.status
    ...
    def test_config_connection_pool_defaults(self, config_valid):
